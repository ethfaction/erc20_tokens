...
function symbol_to_details(json, dest_file_path) {
    ...
    json.forEach(token => {
        obj[token.symbol] = {
            ...
            "contract_abi": token.contract_abi
        }
    });
...
function contract_to_abi(json, dest_file_path) {
    obj = {}

    json.forEach(token => {
        obj[token.contract_address] = token.contract_abi
    });
...
contract_to_abi(json_data, 'docs/mainnet/contract_to_abi/index.json');
